...
$limit = 50;
$offset = ($page-1) * $limit;
$baza = mysqli_query($db, "SELECT COUNT(*) AS ct FROM `debt` WHERE 
debt.rDate > now() AND debt.rDate < date_add(now(), INTERVAL 7 day) AND debt.iduser = '$uid'");

$count_all = mysqli_fetch_assoc($baza)['ct'];
$total_pages = ceil($count_all / $limit);


$xrow = mysqli_query($db, "SELECT * FROM debt WHERE  debt.rDate > now() AND debt.rDate < date_add(now(), INTERVAL 7 day) 
AND debt.iduser = '$uid' ORDER BY debt.rDate ASC LIMIT $offset, $limit ");
